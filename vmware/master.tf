resource "vsphere_virtual_machine" "master" {
  count            = "${var.master_node_instance_count}"
  name             = "${var.datacenter_code}-${var.env_code}-m${count.index}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.datacenter_code}-${var.env_code}-m${count.index}"
        domain    = "${var.dns_suffix_list[0]}"
      }

      network_interface {
        ipv4_address = "${var.ip2}"
...
resource "null_resource" "master_testing" {
    count = "${var.master_node_instance_count}"
    provisioner "local-exec" {
        command = <<EOT
          echo ${element(vsphere_virtual_machine.master.*.default_ip_address, count.index)}
        EOT
    }
}