resource "vsphere_virtual_machine" "infra" {
  count            = "${var.infra_node_instance_count}"
  name             = "${var.datacenter_code}-${var.env_code}-i${count.index}"
...
resource "null_resource" "infra_testing" {
    count = "${var.infra_node_instance_count}"
    provisioner "local-exec" {
        command = <<EOT
          echo ${element(vsphere_virtual_machine.infra.*.default_ip_address, count.index)}
        EOT
    }
}